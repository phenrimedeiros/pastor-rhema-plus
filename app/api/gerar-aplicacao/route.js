...
export async function POST(request) {
  ...
  let record;
  try {
    record = await saveVersionedSermonContent(supabase, weekId, "application", content);
  } catch (err) {
    return Response.json({ error: "Falha ao salvar aplicações: " + err.message }, { status: 500 });
  }
