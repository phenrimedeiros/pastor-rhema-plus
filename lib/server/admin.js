...
export const DISABLED_USER_BAN_DURATION = "876000h";
...
export function isAuthUserAccessEnabled(authUser) {
...
export async function findAuthUserByEmail(serviceSupabase, email) {
  ...
  // Tenta lookup O(1) via tabela de indice
  const { data: lookup } = await serviceSupabase
  ...
  if (lookup?.user_id) {
    const { data } = await serviceSupabase.auth.admin.getUserById(lookup.user_id);
    if (data?.user) return data.user;
  }

  // Fallback: varre paginada (usuários pré-migração)
  let page = 1;
  while (true) {
    ...
    if (found) {
      ...
      await serviceSupabase
        .from("hotmart_user_emails")
        .upsert({ email: normalizedEmail, user_id: found.id }, { onConflict: "email", ignoreDuplicates: true });
      return found;
    }
...
export function mapAdminUser(authUser, profile) {
  return {
    ...
    accessEnabled: isAuthUserAccessEnabled(authUser),
