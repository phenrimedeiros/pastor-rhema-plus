...
function isSatisfactionSurveySchemaError(error) {
  const message = String(error?.message || "").toLowerCase();
  return (
    error?.code === "42P01" ||
    error?.code === "PGRST205" ||
    message.includes("could not find the table 'public.satisfaction_surveys'") ||
    message.includes('could not find the table "public.satisfaction_surveys"') ||
    message.includes('relation "public.satisfaction_surveys" does not exist') ||
    message.includes('relation "satisfaction_surveys" does not exist')
  );
}
...
export const satisfactionSurvey = {
  async getMyResponse(userId) {
    ...
    if (!error) return { schemaReady: true, response: data || null };

    if (isSatisfactionSurveySchemaError(error)) {
      return { schemaReady: false, response: null };
    }

    throw new Error(error.message);
  },

  async upsertResponse(userId, response) {
    const payload = {
      ...response,
      user_id: userId,
    ...
    if (isSatisfactionSurveySchemaError(error)) {
      throw new Error("A pesquisa ainda nao esta habilitada no banco de dados.");
    }
...
export async function loadFullState(forceRefresh = false) {
  ...
  try {
    ...
    // 1. Obter todas as semanas de todas as séries do usuário de uma vez
    ...
    if (!allWeeks?.length) {
      const seriesWithEmptyWeeks = userSeries.map((s) => ({ ...s, weeks: [] }));
      const result = {
        authenticated: true,
        user,
        profile,
        series: seriesWithEmptyWeeks,
      };
      _stateCache = result;
      _stateCacheTime = now;
      return result;
    ...
    // 2. Obter todo o conteúdo de sermão ativo para todas as semanas mapeadas
    const weekIds = allWeeks.map((w) => w.id);
    const { data: allActiveContent, error: contentError } = await supabase
      .from("sermon_content")
      .select("*")
      .in("week_id", weekIds)
      .eq("is_active", true)
    ...
    // 3. Agrupar os conteúdos por week_id e step para acesso rápido O(1)
    const contentMap = {};
    (allActiveContent || []).forEach((row) => {
      if (!contentMap[row.week_id]) {
        contentMap[row.week_id] = {};
      }
      if (!contentMap[row.week_id][row.step]) {
        contentMap[row.week_id][row.step] = row;
      }
    });

    // 4. Montar semanas com seus conteúdos correspondentes
    const weeksWithContent = allWeeks.map((w) => {
      const weekContent = contentMap[w.id] || {};
      return {
        ...w,
        study: weekContent.study || null,
        builder: weekContent.builder || null,
        illustrations: weekContent.illustrations || null,
        application: weekContent.application || null,
      };
    });

    // 5. Associar semanas às suas respectivas séries
    const seriesWithWeeks = userSeries.map((s) => {
      return {
        ...s,
        weeks: weeksWithContent.filter((w) => w.series_id === s.id),
      };
    });
...
// ============================================
// CHAT MIGRATION (localStorage → DB)
// ============================================
export async function migrateChatFromLocalStorage(userId) {
  if (!userId || typeof window === "undefined") return 0;

  const localThreads = readChatHistoryLocal(userId);
  if (!localThreads.length) return 0;

  const dbThreads = await chatThreads.getThreads(userId);
  if (dbThreads.length > 0) return 0;
  ...
  for (const thread of sortedThreads) {
    ...
    const dbThread = await chatThreads.createThread(userId, thread.title || "");
    await chatMessages.addMessages(
      dbThread.id,
  ...
  if (migrated > 0) {
    window.localStorage.removeItem(`${CHAT_HISTORY_STORAGE_PREFIX}:${userId}`);
  }
  return migrated;
}
...
function readChatHistoryLocal(userId) {
  try {
    const raw = window.localStorage.getItem(`${CHAT_HISTORY_STORAGE_PREFIX}:${userId}`);
    if (!raw) return [];
