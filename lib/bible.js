...
export function lookupRef(ref, lang = "pt") {
  ...
  const result = getVerses(lang, bookIdx, chapter, verseStart, verseEnd);
  if (!result) return null;
  return { ...result, bookIdx };
}

/**
 * Returns the list of all 66 books for a given language, with chapter counts.
 * Used for navigation UI (no verse text returned).
 */
export function getBooks(lang = "pt") {
  const bible = BIBLES[lang] || BIBLES.pt;
  return bible.map((book, idx) => ({
    idx,
    id: book.id,
    name: book.name,
    chapters: book.chapters.length,
    testament: idx < 39 ? "OT" : "NT",
  }));
}

/**
 * Returns a single chapter by book index (0-based) and chapter number (1-based).
 */
export function getChapter(lang, bookIdx, chapter) {
  return getVerses(lang, bookIdx, chapter, null, null);
}
